 ...
 **********************************************************************************************************************/
#pragma inline(R_RIIC_GetVersion)
...
static riic_return_t riic_after_receive_slvadr (riic_info_t * p_riic_info)
{
    ...
    /* Slave send mode? */
    else if (0x00 != ((*piccr2_reg) & RIIC_ICCR2_TRS))
    {
        /* Sets the internal status. */
        riic_api_status_set(p_riic_info, RIIC_STS_SEND_DATA_WAIT);
...
void riic1_eei_sub(void)
{
    ...
    if ((0U != RIIC1.ICSR2.BIT.STOP) && (0U != RIIC1.ICIER.BIT.SPIE))
    {
        if (((RIIC_MODE_S_READY == riic_api_info[1].N_Mode)||
                        (RIIC_MODE_S_SEND == riic_api_info[1].N_Mode)) ||
...
void riic1_txi_sub(void)
{
    ...
    if((RIIC_ICCR2_MST_SET != (tmp & RIIC_ICCR2_MST)) &&
            (RIIC_ICCR2_TRS_SET == (tmp & RIIC_ICCR2_TRS)))
    {
        /* Processing when the addresses match (slave send specified) */
...
void riic2_eei_sub(void)
{
    ...
    if ((0U != RIIC2.ICSR2.BIT.STOP) && (0U != RIIC2.ICIER.BIT.SPIE))
    {
        if (((RIIC_MODE_S_READY == riic_api_info[2].N_Mode)||
                        (RIIC_MODE_S_SEND == riic_api_info[2].N_Mode)) ||
...
#if (RIIC_CFG_CH3_INCLUDED == 1U)
/***********************************************************************************************************************
 * Function Name: riic3_eei_sub
 * Description  : Interrupt EEI handler for channel 3.
 *                Types of interrupt requests transfer error or event generation.
 *                The event generations are arbitration-lost, NACK detection, timeout detection, 
 *                start condition detection, and stop condition detection.
 * Arguments    : None
 * Return Value : None
 **********************************************************************************************************************/
void riic3_eei_sub(void)
{
    riic_info_t * p_riic_info;

    #ifdef TN_RXA012A
    riic_timeout_counter_clear(3);
    #endif

    /* Checks Time Out. */
    if ((0U != RIIC3.ICSR2.BIT.TMOF) && (0U != RIIC3.ICIER.BIT.TMOIE))
    {
        /* all interrupt disable */
        RIIC3.ICIER.BIT.TMOIE = 0U;
        while (0U != RIIC3.ICIER.BIT.TMOIE)
        {
            /* Do Nothing */
        }
        riic_api_event[3] = RIIC_EV_INT_TMO;
    }
    ...
    if ((0U != RIIC3.ICSR2.BIT.AL) && (0U != RIIC3.ICIER.BIT.ALIE))
    {
        ...
        while (0U != RIIC3.ICIER.BIT.ALIE)
        {
            /* Do Nothing */
        }
        riic_api_event[3] = RIIC_EV_INT_AL;
        if(RIIC_MODE_S_READY == riic_api_info[3].B_Mode)
        {
            riic_set_ch_status(priic_info_s[3], RIIC_COMMUNICATION);
        }
    }
    ...
    {
        if (((RIIC_MODE_S_READY == riic_api_info[3].N_Mode)||
                        (RIIC_MODE_S_SEND == riic_api_info[3].N_Mode)) ||
                (RIIC_MODE_S_RECEIVE == riic_api_info[3].N_Mode))
        {
            /* Sets the internal status. */
            riic_api_status_set(priic_info_s[3], RIIC_STS_SP_COND_WAIT);
        }

        /* Prohibits STOP interrupt. */
        RIIC3.ICIER.BIT.SPIE = 0U;

        RIIC3.ICMR3.BIT.RDRFS = 0U; /* Refer to the technical update. */
        RIIC3.ICMR3.BIT.ACKWP = 1U; /* Refer to the technical update. */
        RIIC3.ICMR3.BIT.ACKBT = 0U; /* Refer to the technical update. */
        RIIC3.ICMR3.BIT.ACKWP = 0U; /* Refer to the technical update. */
        while ((0U != RIIC3.ICMR3.BIT.RDRFS) || (0U != RIIC3.ICMR3.BIT.ACKBT))
        {
            /* Do Nothing */
        }

        /* Clears each status flag. */
        RIIC3.ICSR2.BIT.STOP = 0U;
        while (0U != RIIC3.ICSR2.BIT.STOP)
        {
            /* Do Nothing */
        }

        /* Sets event flag. */
        riic_api_event[3] = RIIC_EV_INT_STOP;
    }
    ...
    if ((0U != RIIC3.ICSR2.BIT.NACKF) && (0U != RIIC3.ICIER.BIT.NAKIE))
    {
        ...
        if (0U == RIIC3.ICCR2.BIT.TRS)
        {
            riic_mcu_clear_ir_rxi(3);
        }

        /* Sets event flag. */
        riic_api_event[3] = RIIC_EV_INT_NACK;
    }
    ...
    if ((0U != RIIC3.ICSR2.BIT.START) && (0U != RIIC3.ICIER.BIT.STIE))
    {
        ...
        while ((0U != RIIC3.ICSR2.BIT.START) || (0U != RIIC3.ICIER.BIT.STIE))
        {
            /* Do Nothing */
        }

        /* Sets event flag. */
        riic_api_event[3] = RIIC_EV_INT_START;
    }

    switch(riic_api_info[3].N_Mode)
    {
        case RIIC_MODE_M_SEND:
        case RIIC_MODE_M_RECEIVE:
        case RIIC_MODE_M_SEND_RECEIVE:
        /* Master mode data */
        p_riic_info = priic_info_m[3];
        break;
    ...
    r_riic_advance(p_riic_info); /* Calls advance function */
} /* End of function riic3_eei_sub() */

/***********************************************************************************************************************
 * Outline      : 
 * Function Name: riic3_txi_sub
 * Description  : Interrupt TXI handler for channel 3.
 *                Types of interrupt requests transmission data empty. Does not use.
 * Arguments    : None
 * Return Value : None
 **********************************************************************************************************************/
void riic3_txi_sub(void)
{
    uint8_t tmp;
            ...
            (RIIC_ICCR2_TRS_SET == (tmp & RIIC_ICCR2_TRS)))
    {
        ...
        /* When an arbitration-lost is detected, the riic_after_receive_slvadr function is not  */
        /* executed. Thus the internal status needs to be changed to "RIIC_STS_SEND_DATA_WAIT". */
        if (RIIC_STS_AL == riic_api_info[3].N_status)
        {
            /* Processing when an arbitration-lost is detected. */
            /* Sets the internal status. */
            riic_api_status_set(priic_info_s[3], RIIC_STS_SEND_DATA_WAIT);
        }
        ...
        if (RIIC_MODE_S_READY == riic_api_info[3].N_Mode)
        {
            /* Processing for the first slave transmission */
            /* Sets the internal mode. */
            riic_api_info[3].N_Mode = RIIC_MODE_S_SEND; /* Set slave transmission mode. */
        }

        /* Sets event. */
        switch (riic_api_info[3].N_status)
        {
            case RIIC_STS_IDLE_EN_SLV:
            ...
            /* Sets interrupted address sending. */
            riic_api_event[3] = RIIC_EV_INT_ADD;
            ...
            /* Sets interrupted data sending. */
            riic_api_event[3] = RIIC_EV_INT_SEND;
        ...
        r_riic_advance(priic_info_s[3]); /* Calls advance function */

    }
} /* End of function riic3_txi_sub() */

/***********************************************************************************************************************
 * Function Name: riic3_rxi_sub
 * Description  : Interrupt RXI handler for channel 3.
 *                Types of interrupt requests reception end.
 * Arguments    : None
 * Return Value : None
 **********************************************************************************************************************/
void riic3_rxi_sub(void)
{
    riic_info_t * p_riic_info;
    ...
    {
        /* Sets the internal status. */
        riic_api_status_set(priic_info_s[3], RIIC_STS_IDLE_EN_SLV);
    }

    if (RIIC_MODE_S_READY == riic_api_info[3].N_Mode)
    {
        /* Sets the internal mode. */
        riic_api_info[3].N_Mode = RIIC_MODE_S_RECEIVE;
    }

    if ((RIIC_STS_IDLE_EN_SLV == riic_api_info[3].N_status))
    {
        /* Updates the channel status. */
        riic_set_ch_status(priic_info_s[3], RIIC_COMMUNICATION);
    }
    ...
    /* Sets interrupted data receiving. */
    riic_api_event[3] = RIIC_EV_INT_RECEIVE;

    r_riic_advance(p_riic_info); /* Calls advance function */
} /* End of function riic3_rxi_sub() */

/***********************************************************************************************************************
 * Function Name: riic3_tei_sub
 * Description  : Interrupt TEI handler for channel 3.
 *                Types of interrupt requests transmission end.
 * Arguments    : None
 * Return Value : None
 **********************************************************************************************************************/
void riic3_tei_sub(void)
{
    #ifdef TN_RXA012A
    riic_timeout_counter_clear(3);
    #endif
    ...
    r_riic_advance(priic_info_m[3]); /* Calls advance function */
} /* End of function riic3_tei_sub() */
#endif
